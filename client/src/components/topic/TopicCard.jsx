...
export default function TopicCard({ topic, key }) {
    ...
    return (
        ...
        // <div key={key} className='flex justify-start items-center gap-4 bg-gray-300 m-2 md:my-1 rounded-lg shadow-lg shadow-gray-200' onClick={() => navigateToPage(topic)}>
        //     <div className='min-h-16 min-w-16 flex justify-center items-center bg-gray-200 text-2xl font-bold font-sans rounded-lg'>{TextToAvatarText(topic?.title)}</div>
        //     <div className='flex justify-between items-center w-full px-4'>
        //         <div className='text-2xl font-medium'>{topic?.title}</div>
        //     </div>
        // </div>
    )
}
